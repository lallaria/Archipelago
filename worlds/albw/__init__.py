...
from .Hints import GhostHint, sanitize, generate_hints, generate_bow_of_light_hint
...
class ALBWWorld(World):
    ...
    hints: Optional[List[GhostHint]] = None
    bow_of_light_hint: Optional[GhostHint] = None
    _hints_lock: ClassVar[threading.Lock] = threading.Lock()
    ...
    def fill_slot_data(self) -> Dict[str, Any]:
        ...
        slot_data["ghost_hints"] = [[hint.location.player, hint.location.address]
            if hint.location and hint.location.address is not None else None
            for hint in [*hints, bow_of_light_hint]]
        return slot_data
    ...
    def _get_hints(self) -> Tuple[List[GhostHint], GhostHint]:
        # must run after progression balancing; generate_output and fill_slot_data race for it on separate threads
        with self._hints_lock:
            if self.hints is None:
                self.hints = generate_hints(self.multiworld, self.player, self.options, self.random)
                self.bow_of_light_hint = generate_bow_of_light_hint(self.multiworld, self.player)
        return self.hints, self.bow_of_light_hint
