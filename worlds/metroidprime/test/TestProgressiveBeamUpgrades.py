    ...
    def test_progressive_items_are_added_to_pool(self):
        ...
        for upgrade in ProgressiveUpgrade:
            if upgrade == ProgressiveUpgrade.Progressive_Power_Beam:
                self.assertTrue(counts[upgrade.value] == 2, f"{upgrade.value} had: {counts[upgrade.value]}")
            else:
                self.assertTrue(counts[upgrade.value] == 3, f"{upgrade.value} had: {counts[upgrade.value]}")
