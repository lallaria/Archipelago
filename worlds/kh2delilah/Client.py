...
class KH2CommandProcessor(ClientCommandProcessor):
    ...
    def _cmd_deathlink(self):
        """Toggles deathlink"""
        if isinstance(self.ctx, KH2DelilahContext):
...
class KH2DelilahContext(CommonContext):
    ...
    def __init__(self, server_address, password):
        super(KH2DelilahContext, self).__init__(server_address, password)
        ...
        # EPIC GAMES addresses
        self.Now = 0x0716DF8
        self.Save = 0x09A92F0
        self.Journal = 0x743260
        self.Shop = 0x743350
        self.Slot1 = 0x2A22FD8
        self.Death = 0x0ABB2B8
        self.CtrlrInpt = 0x2A22FD8
        # self.Sys3 = 0x2A59DF0
        # self.Bt10 = 0x2A74880
        ...
        self.nothingness_list = [
            ...
            {"text": "o", "type": "color", "color": "#c46262"},
            {"text": "t", "type": "color", "color": "#c57b7b"},
            {"text": "h", "type": "color", "color": "#c59494"},
            {"text": "i", "type": "color", "color": "#c6adad"},
            {"text": "n", "type": "color", "color": "#c6c6c6"},
            {"text": "g", "type": "color", "color": "#c2c2c2"},
            {"text": "n", "type": "color", "color": "#bdbdbd"},
            {"text": "e", "type": "color", "color": "#b9b9b9"},
            {"text": "s", "type": "color", "color": "#b5b5b5"},
            {"text": "s ", "type": "color", "color": "#b0b0b0"}, 
            {"text": "i", "type": "color", "color": "#aeaa9e"},
            {"text": "s ", "type": "color", "color": "#b0a890"}, 
            {"text": "E", "type": "color", "color": "#b5a375"},
            {"text": "t", "type": "color", "color": "#b7a167"},
            {"text": "e", "type": "color", "color": "#aca55f"},
            {"text": "r", "type": "color", "color": "#a2a957"},
            {"text": "n", "type": "color", "color": "#97ad4e"},
            {"text": "a", "type": "color", "color": "#8db146"},
            {"text": "l", "type": "color", "color": "#82b53e"}
        ]
        self.darkness_list = ["Darkness", "C'mon Sora, I thought you were stronger than that.", 
                              "Looks like my summer vacation is over.","Submit!",
                              "Your heart succumbs, your body submits, so why is it that your mind resists?",
                              "Anger and hate are supreme.","A meaningless effort.",
                              "...But who will I have ice cream with?","Silly. Just because you have a next life..."]

    async def server_auth(self, password_requested: bool = False):
        if password_requested and not self.password:
            await super(KH2DelilahContext, self).server_auth(password_requested)
    ...
    async def connection_closed(self):
        ...
        if self.kh2seedname is not None and self.auth is not None:
            with open(os.path.join(self.game_communication_path, f"kh2delilahsave2{self.kh2seedname}{self.auth}.json"),
                    'w') as f:
                f.write(json.dumps(self.kh2_seed_save, indent=4))
        await super(KH2DelilahContext, self).connection_closed()
    ...
    async def disconnect(self, allow_autoreconnect: bool = False):
        ...
        if self.kh2seedname not in {None} and self.auth not in {None}:
            with open(os.path.join(self.game_communication_path, f"kh2delilahsave2{self.kh2seedname}{self.auth}.json"),
                    'w') as f:
                f.write(json.dumps(self.kh2_seed_save, indent=4))
        await super(KH2DelilahContext, self).disconnect()
    ...
    def on_deathlink(self, data: dict):
        if "DeathLink" in self.tags:
            Deathcall = DTD(self.kh2)
            sora_pointer = Deathcall.return_me(self.game_func_addrs["SoraPointer"] + self.kh2.base_address)
            Deathcall.void_me(self.game_func_addrs["HitPointFunc"] + self.kh2.base_address, sora_pointer, -120, 0, True)
            super().on_deathlink(data)

    async def checkDeath(self):
        sora_dies = self.kh2_read_short(self.Death)
        if not (sora_dies == 4 or sora_dies == 5):
            self.onContinueScreen = False
        if (sora_dies == 4 or sora_dies == 5) and not self.onContinueScreen and "DeathLink" in self.tags:
            self.onContinueScreen = True
            await self.send_death(choice(self.darkness_list), self.nothingness_list)
    ...
    def on_package(self, cmd: str, args: dict):
        ...
        if cmd in {"Connected"}:
            asyncio.create_task(self.send_msgs([{"cmd": "GetDataPackage", "games": ["Kingdom Hearts 2 Delilah"]}]))
...
async def kh2_watcher(ctx: KH2DelilahContext):
    while not ctx.exit_event.is_set():
        try:
            if ctx.kh2connected and ctx.serverconneced:
                ...
                await asyncio.create_task(ctx.checkDeath())#deathlink
                    ...
                    if ctx.kh2 is not None:
                        if ctx.kh2_game_version is None:
                            if ctx.kh2_read_string(0x09A9830, 4) == "KH2J":
                                ...
                                ctx.Shop = 0x7435D0
                                ctx.Death = 0x0ABB7F8
                                ctx.CtrlrInpt = 0xBF31F0

                            elif ctx.kh2_read_string(0x09A92F0, 4) == "KH2J":
                                ctx.kh2_game_version = "Epic Games"
                            else:
                                ctx.kh2_game_version = None
                                logger.info("Your game version is out of date. Please update your game via The Epic Games Store or Steam.")
                        if ctx.kh2_game_version is not None:
                            logger.info(f"You are connected to Kingdom Hearts 2 Final Mix on {ctx.kh2_game_version}")
...
def launch():
    async def main(args):
        ctx = KH2DelilahContext(args.connect, args.password)
    ...
    parser = get_base_parser(description="KH2 Delilah Client, for text interfacing.")
