...
def patch_kh2(self, output_directory):
    ...
    mod_name = f"AP-{self.multiworld.get_file_safe_player_name(self.player)}-P{self.player}-{mytimestamp}"
    ...
    for location in self.multiworld.get_filled_locations(self.player):
        ...
        if location.item:
            if location.item.player == self.player:
                itemcode = item_dictionary_table[location.item.name].kh2delilahid
            else:
        ...
        if location in levelsetting:
            ...
            if data.item:
                if data.item.player == self.player:
                    itemcode = item_dictionary_table[data.item.name].kh2delilahid
                else:
                    itemcode = 90  # castle map
        else:
            increaseStat(self.random.randint(0, 3))
    ...
    mod_dir = os.path.join(output_directory, mod_name + self.multiworld.seed_name + "_" + Utils.__version__)

    self.mod_yml["title"] = f"Archipelago Seed - {self.multiworld.get_file_safe_player_name(self.player)}"
    self.mod_yml["originalAuthor"] = "JaredWeakStrike and DelilahIsDidi"
